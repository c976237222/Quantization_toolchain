...
def custom_dataloader(data_dir: str, val_file: str, batch_size: int, data_extension: str = '.jpg') -> Iterable[Tuple[torch.Tensor, torch.Tensor]]:
    """
    Custom dataloader to load image data and labels for evaluation.

    Args:
        data_dir (str): Directory containing the image files.
        val_file (str): Path to the validation label file.
        batch_size (int): Number of samples per batch.
        data_extension (str): File extension for image files (default: '.jpg').
    ...
    # Define image transformations (resize and normalization for VGG-like models)
    ...
    for i in range(0, len(matched_files), batch_size):
        ...
        data_batch = [transform(Image.open(file).convert('RGB')) for file in batch_files]
...
def evaluate_quantized_model(graph, data_dir: str, val_file: str, batch_size: int, device: str = 'cuda') -> float:
    ...
    Args:
        ...
        data_dir (str): Path to the image files.
        val_file (str): Path to the validation label file.
        batch_size (int): Number of samples per batch.
