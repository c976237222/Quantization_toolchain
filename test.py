...
graph = None
output_onnx = os.path.join(WORKING_DIRECTORY, 'vgg_fp32.onnx')
output_config = os.path.join(WORKING_DIRECTORY, 'vgg_fp32_cfg.json')
output_engine = os.path.join(WORKING_DIRECTORY, 'vgg_fp16.engine')
...
if need_compile_model:
    ...
    builder.create_engine(engine_path=output_engine, precision="fp16")
    check_dynamic_batch(output_engine)

if need_accuracy:
    print("计算量化后模型精确度")
    result = evaluate_quantized_model(quantized, "working/data", label_path, 16)
    result1 = evaluate_vgg16_model("working/data", label_path, 16)
...
if need_performance:
    BATCHSIZE = 128
    print("测试engine模型精确度")
    evaluate_quantized_model_trt('vgg_fp32.engine', "working/data", label_path, BATCHSIZE)
    evaluate_quantized_model_trt('vgg_fp16.engine', "working/data", label_path, BATCHSIZE)
    evaluate_quantized_model_trt('vgg_int8.engine', "working/data", label_path, BATCHSIZE)
    print(f"测试engine模型推理速度,batchsize{BATCHSIZE}")

    from ppq.utils.TensorRTUtil import build_engine, Benchmark, Profiling
    Benchmark("int8.engine")
